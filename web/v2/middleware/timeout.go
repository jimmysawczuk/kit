	...
	return func(h http.Handler) http.Handler {
		return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			case <-time.After(timeout):
				tw.mx.Lock()
				respond.WithError(ctx, tw, r, http.StatusGatewayTimeout, fmt.Errorf("timed out after %s", timeout))
				tw.done = true
				tw.mx.Unlock()
			case perr := <-panicCh:
				tw.mx.Lock()
				respond.WithError(ctx, tw, r, http.StatusInternalServerError, perr)
				tw.done = true
				tw.mx.Unlock()
			}
		})
